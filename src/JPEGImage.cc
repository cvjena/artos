...
JPEGImage::JPEGImage(FILE * filehandle) : width_(0), height_(0), depth_(0)
{
    if (!filehandle)
        return;
...
FeatureMatrix_<uint8_t> JPEGImage::toMatrix()
{
    return FeatureMatrix_<uint8_t>(empty() ? nullptr : &bits_[0], height_, width_, depth_);
}

const FeatureMatrix_<uint8_t> JPEGImage::toMatrix() const
{
    return FeatureMatrix_<uint8_t>(empty() ? nullptr : const_cast<uint8_t*>(&bits_[0]), height_, width_, depth_);
}
...
JPEGImage JPEGImage::crop(int x, int y, int width, int height) const
{
    ...
    int y2, x2, i, y1offs, x1offs, y2offs, x2offs;
    for (y2 = 0; y2 < height; ++y2)
    {
        y1offs = (y + y2) * width_ * depth_;
        y2offs = y2 * width * depth_;
        for (x2 = 0; x2 < width; ++x2)
        {
            x1offs = y1offs + (x + x2) * depth_;
            x2offs = y2offs + x2 * depth_;
            for (int i = 0; i < depth_; ++i)
                result.bits_[x2offs + i] = bits_[x1offs + i];
        }
    }
...
JPEGImage JPEGImage::cropPadded(int x, int y, int width, int height) const
{
    // Empty image
    if ((width <= 0) || (height <= 0))
        return JPEGImage();
    ...
    int y1, x1, y2, x2, i, y1offs, x1offs, y2offs, x2offs;
    for (y2 = 0; y2 < height; ++y2)
    {
        y1 = y + y2;
        if (y1 < 0)
            y1 *= -1;
        if (y1 >= height_)
            y1 = height_ - 1 - (y1 % height_);
        y1offs = y1 * width_ * depth_;
        y2offs = y2 * width * depth_;
        for (x2 = 0; x2 < width; ++x2)
        {
            x1 = x + x2;
            if (x1 < 0)
                x1 *= -1;
            if (x1 >= width_)
                x1 = width_ - 1 - (x1 % width_);
            x1offs = y1offs + x1 * depth_;
            x2offs = y2offs + x2 * depth_;
            for (i = 0; i < depth_; ++i)
                result.bits_[x2offs + i] = bits_[x1offs + i];
        }
