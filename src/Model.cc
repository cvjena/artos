...
Model::Model(const FeatureMatrix & root, const Scalar bias) : parts_(1), bias_(bias)
{
...
Model::Model(FeatureMatrix && root, const Scalar bias) : parts_(1), bias_(bias)
{
...
{
    return (parts_[0].filter.empty() && !nbParts());
}

int Model::nbFeatures() const
{
    return parts_[0].filter.channels();
}

Size Model::rootSize() const
{
    return Size(parts_[0].filter.cols(), parts_[0].filter.rows());
}
...
Size Model::partSize() const
{
    if (nbParts())
        return Size(parts_[1].filter.cols(), parts_[1].filter.rows());
    else
        return Size(0, 0);
}
...
const FeatureMatrix & Model::filters(std::size_t index) const
{
    return this->parts_[index].filter;
}

Model Model::flip(const shared_ptr<FeatureExtractor> & featureExtractor) const
{
    Model model;
    ...
    if (!empty()) {
        ...
        // Flip the root
        featureExtractor->flip(parts_[0].filter, model.parts_[0].filter);
        model.parts_[0].offset = parts_[0].offset;
        model.parts_[0].deformation = parts_[0].deformation;
        
        // Flip the parts
        for (int i = 1; i < parts_.size(); ++i) {
            featureExtractor->flip(parts_[i].filter, model.parts_[i].filter);
...
void Model::convolve(const FeaturePyramid & pyramid, vector< vector<ScalarMatrix> > & convolutions,
                     ...
                     vector< vector<Positions> > * positions) const
{
    ...
    vector<Scalar> tmp(pyramid.levels()[0].rows() * pyramid.levels()[0].cols());
        ...
        for (int j = 0; j < nbLevels - interval; ++j) {
            ...
            for (int y = 0; y < convolutions[0][j + interval].rows(); ++y) {
                for (int x = 0; x < convolutions[0][j + interval].cols(); ++x) {
                    ...
                    if (x2 < convolutions[i + 1][j].cols() && y2 < convolutions[i + 1][j].rows())
...
void Model::DT2D(ScalarMatrix & matrix, const Model::Part & part, Scalar * tmp, Positions * positions)
{
    ...
    if (positions) {
        Eigen::Map<Eigen::MatrixXi> tmp2(reinterpret_cast<int *>(tmp), rows, cols);
...
ostream & ARTOS::operator<<(ostream & os, const Model & model)
{
    ...
    for (int i = 0; i < model.parts_.size(); ++i) {
        os << model.parts_[i].filter.rows() << ' ' << model.parts_[i].filter.cols() << ' '
           << model.parts_[i].filter.channels() << ' ' << model.parts_[i].offset(0) << ' '
        ...
        for (int y = 0; y < model.parts_[i].filter.rows(); ++y)
        {
            for (int x = 0; x < model.parts_[i].filter.cols(); ++x)
                for (int j = 0; j < model.parts_[i].filter.channels(); ++j)
...
istream & ARTOS::operator>>(istream & is, Model & model)
{
    int nbParts;
    Model::Scalar bias;
    is >> nbParts >> bias;
    ...
    for (int i = 0; i < nbParts; ++i)
    {
        ...
        model.parts_[i].filter = FeatureMatrix(rows, cols, FeatureCell::Zero(nbFeatures));
